...
public class ControlThread extends Thread {
	...
	public ControlThread(Socket client) {
		...
		try {
			...
			controlOutput = new PrintWriter(
					controlConnection.getOutputStream(), true);
	...
	private void sendResponse(int code) {
		...
		case 504:
			controlOutput
					.write("504 Command not implemented for the parameter.\r\n");
		case 530:
	...
	private void listFiles(String folder) throws IOException {
		File file = new File(workingDirectory + "/" + folder);
		System.out.println(workingDirectory + "/" + folder + " requested.");
		if (file.exists()) {
			sendResponse(150);
			ServerSocket dataConnection = new ServerSocket(dataPort);
			Socket data = dataConnection.accept();
